...
{
    [Authorize][AuthorizeRoles("Admin","HOD","Instructor")]
    public class ReportController : Controller
    {
        private readonly IUnitOfWork _uow;
        public ReportController(IUnitOfWork uow) => _uow = uow;

        public async Task<IActionResult> Equipment(int? labId, string? condition)
        {
            ...
            {
                EquipmentId = e.EquipmentId, AssetCode = e.AssetCode, Model = e.Model,
                SerialNumber = e.SerialNumber, PurchaseDate = e.PurchaseDate,
                WarrantyExpiry = e.WarrantyExpiry, Condition = e.Condition, Notes = e.Notes,
                LabId = e.LabId, EquipmentTypeId = e.EquipmentTypeId,
        ...
        public async Task<IActionResult> Complaints(DateTime? from, DateTime? to, string? status, int? labId)
        {
            ...
            {
                ComplaintId = c.ComplaintId, Title = c.Title, Status = c.Status, Priority = c.Priority,
            ...
            ViewBag.Summary = new
            {
                Total = vm.Count, Pending = vm.Count(c => c.Status == "Pending"),
                InProgress = vm.Count(c => c.Status == "InProgress"), Resolved = vm.Count(c => c.Status == "Resolved"),
                Closed = vm.Count(c => c.Status == "Closed")
            };
            return View(vm);
        }

        public async Task<IActionResult> SoftwareExpiry()
        {
            var all = await _uow.Softwares.GetAllAsync();
            var labs = await _uow.Labs.GetAllAsync();
            var threshold = DateTime.Today.AddDays(60);
            var expiring = all.Where(s => s.LicenseExpiry.HasValue && s.LicenseExpiry.Value.Date <= threshold)
            ...
            var vm = expiring.Select(s => new SoftwareViewModel
            {
                ...
                LicenseExpiry = s.LicenseExpiry, LabId = s.LabId, IsNotificationSent = s.IsNotificationSent,
